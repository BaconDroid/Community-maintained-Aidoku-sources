...
mod models;
use helpers::{
	ContentType, body_to_text, decode_manga_key, deep_link_manga_key, fetch_chapters, list_url,
	manga_from_detail, manga_page_result, request, valid_number, valid_slug,
};
...
fn page_list(sort: &str, page: i32, filters: &[FilterValue]) -> Result<MangaPageResult> {
	let content_type = content_type_from_filters(filters);
	let limit = 60u32;
	let offset = (page.max(1) as u32 - 1) * limit;
	let data: ListResponse = request(&list_url(content_type, sort, limit, offset, None, filters))?;
...
impl Source for Chikari {
	...
	fn get_manga_update(
		...
		if needs_details {
			manga.copy_from(manga_from_detail(
				request(&format!(
					"{BASE_URL}{}/{}",
...
fn content_type_from_filters(filters: &[FilterValue]) -> ContentType {
	filters
		.iter()
		.find_map(|filter| match filter {
			FilterValue::Select { id, value } if id == "content_type" => Some(value),
			_ => None,
		})
		.map(|value| match value.as_str() {
			"comics" | "series" => ContentType::Series,
			_ => ContentType::Novel,
		})
		.unwrap_or(ContentType::Novel)
}
...
mod tests {
	...
	#[aidoku_test]
	fn content_type_filter_maps_comics() {
		let filters = vec![FilterValue::Select {
			id: "content_type".into(),
			value: "comics".into(),
		}];
		assert_eq!(content_type_from_filters(&filters), ContentType::Series);
	}
}
